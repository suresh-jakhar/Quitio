...
class GraphQuery:
    ...
    async def get_neighbors(
        ...
        async def traverse(current_id: str, current_depth: int, path: List[dict]):
            # 1. STRICT DEPTH & LATENCY CONTROL
            if current_depth <= 0 or len(path) > 2 or len(visited) > 50:
                return
            
            visited.add(current_id)
            neighbors = await self._get_neighbors_with_centrality(current_id, limit=15)
            ...
            for n in neighbors:
                ...
                if target_id not in visited:
                    await traverse(target_id, current_depth - 1, new_path)

        await traverse(card_id, depth, [])
    ...
    async def search_with_graph(
        ...
        seeds = await vector_store.find_similar_cards(query_embedding, user_id, limit=5)
        ...
        return await self._enrich_card_details(final_results)
    ...
    def _apply_mmr(self, candidates: List[dict], query_embedding: List[float], k: int, lambda_param: float = 0.7) -> List[dict]:
        ...
        while len(selected) < k and remaining:
            ...
            for cand in remaining:
                ...
                redundancy = 0.0
                if selected_embs and 'embedding' in cand and cand['embedding']:
                    c_emb = np.array(cand['embedding'])
                    redundancies = [float(np.dot(c_emb, s_emb)) for s_emb in selected_embs]
            ...
            if best_node:
                ...
                if 'embedding' in best_node and best_node['embedding']:
                    selected_embs.append(np.array(best_node['embedding']))
    ...
    async def _get_neighbors_with_centrality(self, card_id: str, limit: int = 10) -> List[dict]:
    ...
    async def _enrich_card_details(self, results: List[dict]) -> List[dict]:
